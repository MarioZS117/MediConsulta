...
export const getPaciente = async (req, res) => {
  try {
    // Obtener el nombre del paciente desde los parámetros
    ...
    // Buscar los datos del paciente en la colección por nombre
    const paciente = await database.collection("pacientes").findOne({ nombrePaciente: nombrePaciente });
    ...
    // Estructura de respuesta con los datos solicitados
    const datosPaciente = {
      _id: paciente._id,
      ...
      emergencyContact: paciente.emergencyContact || "",
      TipoSangre: paciente.TipoSangre || "",
    };

    // Enviar los datos del paciente como respuesta
    res.status(200).json({ message: "Datos del paciente obtenidos correctamente", datosPaciente });
  } catch (error) {
